...
pub fn load_config() -> anyhow::Result<()> {
    let loaded = config::Config::builder().add_source(config::Environment::with_prefix("ASPEN")).add_source(config::File::new("aspen.toml", config::FileFormat::Toml)).build()?.try_deserialize::<AspenConfig>()?;
...
pub async fn aspen_config() -> AspenConfig {
    CONFIG.read().await.as_ref().expect("config was not yet loaded!").clone()
}